...
- 쿠폰 발급에 필요한 카페24 `member_id`는 `member_joined` 웹훅(event 90083)으로 수신
- 수신된 `member_id`는 `cafe24_members` 테이블에 upsert로 영구 저장 (향후 회원 관리·분석 기능에 활용 가능)
...
| 플랜 | 가격 | 포함 기능 |
|------|------|----------|
| **Free** | 0원 (현재 무료 제공) | 소셜 로그인 9종, 스마트 버튼, 가입 쿠폰 1종, 기본 통계, "Powered by 번개가입" 브랜딩 |
...
> **가격 전략 배경**: v1의 "무료 월 100명 제한"에서 "무료 무제한"으로 전환. 경쟁사 알파푸시가 무료 모델로 6개월 만에 6,000개 쇼핑몰을 확보한 사례를 참고. 소셜 로그인 자체는 무료로 진입 장벽을 없애고, 수익은 가입 유도 부가기능(Plus)에서 창출.
...
**cafe24_members** — 카페24 회원 ID 저장 (member_joined 웹훅, event 90083)
```sql
id         TEXT PRIMARY KEY
shop_id    TEXT NOT NULL  -- FK: shops
mall_id    TEXT NOT NULL
member_id  TEXT NOT NULL  -- 카페24 쇼핑몰 회원 ID
created_at TEXT
updated_at TEXT
