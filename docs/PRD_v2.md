...
| **가격** | 무료(월100명) / 월 29,900원 / 연 329,900원 | **Free(0원) / Plus(월 5,900원)** |
| **무료 범위** | 월 100명 제한 소셜 로그인 | 소셜 로그인 9종 **무제한** + 가입 쿠폰 1종 |
| **서비스 경계** | 소셜 로그인 통합만 | 번개가입(유입) vs otj-shop(운영) 역할 분리 |
| **Phase 1** | 소셜 로그인 + 대시보드 | 소셜 로그인 + 쿠폰 + 배너 + 팝업 + AI 카피 + 카카오 채널 + 에스컬레이션 + 전환 통계 |
...
### 4.3 AI 가입 유도 카피 생성 (월 5회)

- 운영자가 '🤖 AI로 생성' 클릭
- Cloudflare Workers AI (Kimi K2.5)가 쇼핑몰 상품/이벤트 정보 기반으로 카피 3~5개 제안
- 운영자가 선택만 하면 배너/팝업에 즉시 적용
- 월 5회 제공 (추가 사용 시 건당 과금 검토)
...
| **Plus** | 월 5,900원 | 미니 배너, 이탈 감지 팝업, AI 카피 생성(월 5회), 카카오 채널 연동, 재방문 에스컬레이션, 전환 퍼널 통계, 브랜딩 제거 |
...
- Plus 전환: 배너/팝업/AI 카피로 가입률 추가 향상 → ROI 명확 (월 5,900원 대비 가입자 1명의 LTV)
...
| 앱 | 가격 | 소셜 종류 | 번개가입 대응 |
|----|------|----------|-------------|
| **1초가입** | 월 49,900~69,900원 | 카카오싱크 1종 | 소셜 로그인 무료 (9종), Plus 5,900원 |
...
| **AI 카피** | Cloudflare Workers AI (Kimi K2.5) | 월 5회 카피 생성 |
...
[Cloudflare Workers — bg.suparain.kr]
    ...
    │
    ├─ /api/ai/copy (Plus — AI 카피 생성)
    │   └─ Cloudflare Workers AI → 배너/팝업 카피 반환
    │
...
> **v2 스키마 변경 예정**: plan 컬럼의 CHECK 제약을 `('free', 'pro')` 로 변경 필요. 현재 `('free', 'monthly', 'yearly')` 는 v1 가격 구조 기준.
...
- AI 가입 유도 카피 생성 (월 5회, Cloudflare Workers AI)
- 카카오 채널 연동
- 재방문 비회원 에스컬레이션
- 가입 전환 퍼널 통계
- "Powered by" 브랜딩 제거
...
| AI 카피 생성 응답 지연 | 낮음 | 비동기 처리, 대시보드에서 별도 요청 방식 |
...
- AI 카피 생성 시 개인정보 입력 방지 (쇼핑몰 정보만 프롬프트에 사용)
