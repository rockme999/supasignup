...
export async function updateShop(
  db: D1Database,
  shopId: string,
  data: Partial<Pick<Shop, 'shop_name' | 'shop_url' | 'enabled_providers' | 'allowed_redirect_uris' | 'plan' | 'sso_configured' | 'widget_style' | 'platform_access_token' | 'platform_refresh_token'>>,
): Promise<void> {
  const sets: string[] = [];
