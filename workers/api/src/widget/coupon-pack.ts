...
export function getCouponPackJs(): string {
  return [
    ...
    '  };',
    '',
    '  // ─── Plus 업셀 카드 렌더러 (Free 플랜용) ─────────────────',
    '  BGWidget.prototype.renderPlusUpsellCard = function(config) {',
    '    // CSS keyframes 1회만 주입 (renderCouponPackCard와 공유)',
    '    if (!document.getElementById("bg-cp-style")) {',
    '      var usSt = document.createElement("style");',
    '      usSt.id = "bg-cp-style";',
    `      usSt.textContent = ${JSON.stringify(COUPON_PACK_CSS)};`,
    '      document.head.appendChild(usSt);',
    '    }',
    '    var self = this;',
    '    var upsellUrl = (config && config.plus_upsell_url) ? config.plus_upsell_url : "/pricing";',
    '    // 시안 #2(브랜드 핑크, 정적) 카드 + 반투명 오버레이 + 자물쇠 + CTA',
    '    var maskId = "bg-cp-us";',
    `    var cardHtml = '<div style="position:relative;width:300px;height:140px;flex-shrink:0;border-radius:12px;overflow:hidden">'`,
    `      + '<svg width="300" height="140" viewBox="0 0 300 140" xmlns="http://www.w3.org/2000/svg" style="position:absolute;inset:0;width:100%;height:100%;display:block">'`,
    `      + '<defs><linearGradient id="bg-cp-us-g" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stop-color="#db2777"/><stop offset="50%" stop-color="#ec4899"/><stop offset="100%" stop-color="#f472b6"/></linearGradient>'`,
    `      + '<mask id="' + maskId + '"><rect width="300" height="140" fill="white"/><circle cx="0" cy="70" r="13" fill="black"/><circle cx="300" cy="70" r="13" fill="black"/></mask></defs>'`,
    `      + '<rect width="300" height="140" rx="12" fill="url(#bg-cp-us-g)" mask="url(#' + maskId + ')"/></svg>'`,
    `      + '<div style="position:absolute;inset:0;border-left:1.5px dashed rgba(255,255,255,0.25);border-right:1.5px dashed rgba(255,255,255,0.25);margin:0 11px;pointer-events:none;z-index:1"></div>'`,
    `      + '<div style="position:absolute;right:16px;top:50%;transform:translateY(-50%);pointer-events:none;z-index:2;filter:drop-shadow(0 0 8px rgba(255,220,40,0.7))">'`,
    `      + '<svg width="52" height="62" viewBox="0 0 52 62" fill="none"><path d="M30 2L6 34H22L18 60L46 26H30L34 2Z" fill="#FFE033" stroke="#FFB800" stroke-width="1.5" stroke-linejoin="round"/></svg></div>'`,
    `      + '<div style="width:100%;padding-left:24px;padding-right:72px;position:relative;z-index:3">'`,
    `      + '<div style="font-size:34px;font-weight:800;letter-spacing:-0.5px;line-height:1;margin-bottom:6px;color:#fff">\\u20a955,000 \\uc0c1\\ub2f9</div>'`,
    `      + '<div style="font-size:13px;font-weight:600;margin-bottom:4px;color:rgba(255,255,255,0.92)">\\uc2e0\\uaddc \\ud68c\\uc6d0 \\uc6f0\\ucf64 \\ud560\\uc778 \\ucfe0\\ud3f0\\ud329</div>'`,
    `      + '<div style="font-size:10px;opacity:0.82;color:rgba(255,255,255,0.72)">\\ubc88\\uac1c\\uac00\\uc785, \\ubc88\\uac1c\\uc9c0\\uae09 5\\uc7a5</div>'`,
    `      + '</div>'`,
    // 반투명 회색 오버레이
    `      + '<div style="position:absolute;inset:0;background:rgba(15,23,42,0.62);border-radius:12px;z-index:4;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:6px">'`,
    // 자물쇠 아이콘
    `      + '<svg width="28" height="28" viewBox="0 0 24 24" fill="none" style="margin-bottom:2px"><rect x="5" y="11" width="14" height="10" rx="2" fill="rgba(255,255,255,0.9)"/><path d="M8 11V7a4 4 0 0 1 8 0v4" stroke="rgba(255,255,255,0.9)" stroke-width="2" stroke-linecap="round"/></svg>'`,
    `      + '<div style="font-size:13px;font-weight:700;color:#fff;text-align:center;padding:0 16px">Plus \\uac00\\uc785 \\uc2dc \\u20a955,000 \\ucfe0\\ud3f0\\ud329 \\uc790\\ub3d9 \\uc9c0\\uae09</div>'`,
    `      + '</div>'`,
    `      + '</div>';`,
    '    var wrap = document.createElement("div");',
    '    wrap.style.cssText = "display:flex;justify-content:center;margin:16px 0 8px";',
    '    wrap.innerHTML = cardHtml;',
    '    // CTA 버튼 (카드 아래 별도 버튼으로 삽입)',
    '    var ctaWrap = document.createElement("div");',
    '    ctaWrap.style.cssText = "display:flex;justify-content:center;margin:0 0 8px";',
    '    var ctaBtn = document.createElement("button");',
    `    ctaBtn.textContent = "Plus \\uc2dc\\uc791\\ud558\\uae30 \\u2192";`,  // "Plus 시작하기 →"
    '    ctaBtn.style.cssText = "display:block;padding:10px 24px;border-radius:10px;background:linear-gradient(135deg,#db2777,#ec4899);color:#fff;font-size:14px;font-weight:700;cursor:pointer;border:none;font-family:-apple-system,BlinkMacSystemFont,Segoe UI,Roboto,sans-serif";',
    '    ctaBtn.addEventListener("click", function() {',
    '      self.trackEvent("widget.plus_upsell_clicked", { source: "upsell_card" });',
